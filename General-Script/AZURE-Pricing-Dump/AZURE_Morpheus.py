...
SKIP_SSL_VERIFY = True  # Enabled to handle self-signed cert issues in production-like environments
...
REQUEST_DELAY = 0.2
MAX_RETRIES = 3
BATCH_SIZE = 100  # Added for batch processing to handle large datasets without overwhelming the API
BATCH_DELAY = 1  # Seconds to sleep between batches
logging.basicConfig(
    level=logging.INFO,
...
class MorpheusClient:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt == MAX_RETRIES - 1:
                    logger.error(f"Request failed after {MAX_RETRIES} attempts: {str(e)} - Response: {getattr(e.response, 'text', 'No response')}")
                    raise
    ...
    def create_price(self, price_data: Dict) -> Optional[int]:
        try:
            # Use flat payload as per API docs (no outer 'price' wrapper)
            flat_data = price_data.get('price', price_data)
            response = self._make_request('POST', '/api/prices', json=flat_data)
            result = response.json()
            if result.get('success'):
                price_id = result.get('id') or result.get('price', {}).get('id')
                logger.info(f"Created price: {flat_data['name']} (ID: {price_id})")
                return price_id
            else:
                logger.error(f"Failed to create price: {result}")
                return None
        except requests.exceptions.RequestException as e:
            logger.error(f"Error creating price: {str(e)} - Response: {getattr(e.response, 'text', 'No response')}")
            return None
        except Exception as e:
    ...
    def update_price(self, price_id: int, price_data: Dict) -> bool:
        try:
            # Use flat payload as per API docs (no outer 'price' wrapper)
            flat_data = price_data.get('price', price_data)
            response = self._make_request('PUT', f'/api/prices/{price_id}', json=flat_data)
            result = response.json()
            if result.get('success'):
                logger.info(f"Updated price: {flat_data['name']} (ID: {price_id})")
                return True
            else:
                logger.error(f"Failed to update price: {result}")
                return False
        except requests.exceptions.RequestException as e:
            logger.error(f"Error updating price {price_id}: {str(e)} - Response: {getattr(e.response, 'text', 'No response')}")
            return False
        except Exception as e:
            logger.error(f"Unexpected error updating price {price_id}: {e}")
            return False
   
    def create_or_update_price(self, price_data: Dict) -> Optional[int]:
        code = price_data['price']['code'] if 'price' in price_data else price_data['code']
    ...
    def create_price_set(self, price_set_data: Dict) -> bool:
        try:
            # Use flat payload as per API docs (no outer 'priceSet' wrapper)
            flat_data = price_set_data.get('priceSet', price_set_data)
            response = self._make_request('POST', '/api/price-sets', json=flat_data)
            result = response.json()
            if result.get('success'):
                logger.info(f"Created price set: {flat_data['name']}")
                return True
            else:
                logger.error(f"Failed to create price set: {result}")
                return False
        except requests.exceptions.RequestException as e:
            logger.error(f"Error creating price set: {str(e)} - Response: {getattr(e.response, 'text', 'No response')}")
            return False
        except Exception as e:
            logger.error(f"Unexpected error creating price set: {e}")
            return False
...
class PricingConverter:
    ...
    def group_prices_for_price_sets(self, morpheus_prices: List[Dict]) -> Dict[str, List[int]]:
        ...
        for price_data in morpheus_prices:
            price = price_data if 'price' not in price_data else price_data['price']
            name = price['name']
    ...
    def create_price_set(self, group_key: str, price_ids: List[int]) -> Dict:
        ...
        if any(keyword in service_lower for keyword in ['storage', 'disk', 'blob']):
            price_set_type = "datastore"  # Updated to valid type based on Morpheus docs
        elif any(keyword in service_lower for keyword in ['virtual', 'compute']):
            price_set_type = "compute_plus_storage"  # Updated to valid snake_case type
        else:
            price_set_type = "compute_plus_storage"  # Fallback to a valid type; avoid 'compute'
...
def sync_azure_pricing():
    ...
    try:
        ...
        for batch_start in range(0, len(all_azure_items), BATCH_SIZE):
            batch_end = batch_start + BATCH_SIZE
            batch = all_azure_items[batch_start:batch_end]
            logger.info(f"Processing batch {batch_start + 1}-{batch_end} of {len(all_azure_items)} items...")
            
            for i, azure_item in enumerate(batch, start=batch_start):
                try:
...
def test_azure_only():
    ...
    try:
        ...
        if test_items:
            ...
            logger.info("="*50)
            if morpheus_prices:
                sample_morpheus = morpheus_prices[0] if 'price' not in morpheus_prices[0] else morpheus_prices[0]['price']
