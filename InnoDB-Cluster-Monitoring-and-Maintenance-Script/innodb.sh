...
monitor_health_status() {
    ...
    mysqlsh --uri "${MYSQL_USER}@${MYSQL_HOST}" --password="$MYSQL_PASSWORD" --js -e "var cluster = dba.getCluster(); cluster.status({extended: 1});" || handle_error "Failed to get cluster status"
}

# Function to clean bin log
clean_bin_log() {
    echo "Cleaning Binary Logs older than $BIN_LOG_RETENTION_DAYS days:"
    mysql -u "$MYSQL_USER" -h "$MYSQL_HOST" -p"$MYSQL_PASSWORD" -e "PURGE BINARY LOGS BEFORE DATE_SUB(NOW(), INTERVAL $BIN_LOG_RETENTION_DAYS DAY);" || handle_error "Failed to purge binary logs"
}
